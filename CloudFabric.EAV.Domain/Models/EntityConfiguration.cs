...
public class EntityConfiguration : AggregateBase
{
    ...
    public void UpdateAttrributeExternalValues(Guid attributeId, List<object> values)
    {
        EntityConfigurationAttributeReference? attributeReference =
            Attributes.FirstOrDefault(a => a.AttributeConfigurationId == attributeId);
    ...
    public void On(EntityConfigurationNameUpdated @event)
    {
        ...
        LocalizedString? name = newCollection.FirstOrDefault(n => n.CultureInfoId == @event.CultureInfoId);

        if (nameIndex == -1)
        {
            newCollection.Add(new LocalizedString { CultureInfoId = @event.CultureInfoId, String = @event.NewName }
            );
        }
        else
        {
    ...
    {
        EntityConfigurationAttributeReference attributeReference =
            Attributes.First(a => a.AttributeConfigurationId == @event.AttributeConfigurationId);
